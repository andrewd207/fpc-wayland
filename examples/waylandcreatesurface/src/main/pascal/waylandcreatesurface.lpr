...
begin
  ...
  if surface = nil then
  begin
    WriteLn('Can''t create surface');
    Halt(1);
  end;
  WriteLn('Created surface');

  if shell = nil then
  begin
    WriteLn('Can''t find shell');
    Halt(1);
  end;

  shell_surface := shell.GetShellSurface(surface);
  if shell_surface = nil then
  begin
    WriteLn('Can''t create shell surface');
    Halt(1);
  end;
  WriteLn('Created shell surface');
